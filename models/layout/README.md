...
当前主干为v7有效电阻修正版，输入链为v13_22 → M04 v4 → 终端v5h → O型输入v6c → 电阻修正v7。具体路径及提交范围见[Git提交与复现说明](../../docs/Git提交与复现说明_20260912.md)。不是目录名数字最大的文件就自动成为采用版。

v5h/v6c的旧终端约24 Ω，不能省略最后的 `fix_htr_effective_resistance_klink.py`。旧 `recheck_release_layout.py`/`audit_merged_block.py`假定原MUX和双端GSG，不能未经适配用于v7签核。
...
`.lydrc` 文件及原生检查只是项目已实现的规则子集，不是代工官方全套检查。当前M04单元的线宽/间距标记为0；旧40+40属于历史MUX。单元规则检查通过仍不等于全片生产签核。
